...
- 全站底部的全局 AI 助手不会阻塞当前页面；它会按路由返回中文引导，登录后提供高级建议，未登录时优先提示登录。
...
5. 移动端保持独立单列，不牺牲可读性。
6. 页面右下角是全局 AI 助手入口，可从任意路由获得当前场景、检索和来源管理的下一步建议；关闭后焦点会返回启动按钮。
7. 全站使用较大的连续圆角，分类导航保持主导航形态，不使用标签胶囊。
8. 前端页面壳只引用一次 `/static/styles.css`，主头部使用 `site-header` 语义类，方便全局样式和后续主题统一维护。
...
默认入口是 `src/main.py`，静态壳、脚本和样式独立保存在 `frontend/` 中。
